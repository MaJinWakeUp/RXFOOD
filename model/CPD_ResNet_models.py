...
class CPD_ResNet(nn.Module):
    ...
    def __init__(self, in_channels=3, channel=32):
        super(CPD_ResNet, self).__init__()
        self.resnet = B2_ResNet(in_channels=in_channels)
...
class CPD_Head(nn.Module):
    # resnet based encoder decoder
    def __init__(self, in_channels=3, channel=32):
        super(CPD_Head, self).__init__()
        self.resnet = B2_ResNet(in_channels=in_channels)
        self.rfb2_1 = RFB(512, channel)
        self.rfb3_1 = RFB(1024, channel)
        self.rfb4_1 = RFB(2048, channel)
...
class CPD_Tail(nn.Module):
    # resnet based encoder decoder
    def __init__(self, channel=32):
        super(CPD_Tail, self).__init__()
        self.agg2 = aggregation(channel)
        self.upsample = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)

    def forward(self, x2_2, x3_2, x4_2):
        detection_map = self.agg2(x4_2, x3_2, x2_2)
