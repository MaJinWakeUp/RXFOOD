...
from model.resnet import build_backbone
...
class FPNBackbone(nn.Module):
    ...
    def __init__(self, back_bone='resnet101', in_channels=3, pretrained=True):
        super().__init__()
        self.back_bone = build_backbone(back_bone=back_bone, input_channels=in_channels, pretrained=pretrained)
...
class FPNModule(nn.Module):
    def __init__(self, in_channels_list=[256, 512, 1024, 2048], out_channels=256):
        ...
        # Top layer
        self.toplayer = nn.Conv2d(in_channels_list[-1], out_channels, kernel_size=1, stride=1, padding=0)  # Reduce channels

        # Lateral layers
        self.latlayer1 = nn.Conv2d(in_channels_list[2], out_channels, kernel_size=1, stride=1, padding=0)
        self.latlayer2 = nn.Conv2d(in_channels_list[1], out_channels, kernel_size=1, stride=1, padding=0)
        self.latlayer3 = nn.Conv2d(in_channels_list[0], out_channels, kernel_size=1, stride=1, padding=0)
...
class FPNSemantic(nn.Module):
    def __init__(self, num_classes, in_channels=256):
        super().__init__()
        # Semantic branch
        self.semantic_branch = nn.Conv2d(in_channels, 128, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(128, num_classes, kernel_size=1, stride=1, padding=0)
        # num_groups, num_channels
        self.gn1 = nn.GroupNorm(128, 128) 
        self.gn2 = nn.GroupNorm(in_channels, in_channels)
...
if __name__ == "__main__":
    model = FPN([2,4,23,3], 32, back_bone="resnet101")
