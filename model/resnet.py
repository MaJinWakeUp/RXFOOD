...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, input_channels=3, pretrained=True):
...
def ResNet101(input_channels=3, pretrained=True):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], input_channels=input_channels, pretrained=pretrained)
    return model

def build_backbone(back_bone, input_channels=3, pretrained=True):
    if back_bone == "resnet101":
        return ResNet101(input_channels=input_channels, pretrained=pretrained)
